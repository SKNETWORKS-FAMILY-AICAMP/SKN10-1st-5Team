...
배경 이미지를 삽입할때, 이미지 파일을 불러오기 위한 경로설정에 문제가 있었습니다. 각자의 로컬 이미지 저장 장소는 다르기 때문에, 개인 이미지 주소를 파일에 작성한채로 깃허브에 머지하였더니 다른 팀원분들은 에러가 발생하였습니다. 깃허브에 이미지 파일도 같이 커밋해서 팀원분들이 각자 경로를 지정할수 있게 하여 해당 문제를 해결했습니다. </br>
또 다른 문제로는 배경이미지의 색이 너무 뚜렷하여 faq나 등록정보의 레이어들이 잘 보이지 않는것이 있었습니다. 배경을 흐리게 하는 기능도 streamlit에 있었습니다만, 레이어들과 그래프도 같이 투명도가 변하였기에, 로컬 저장소에서 이미지의 투명도를 먼저 낮춘다음 불러오는 방식으로 문제를 해결했습니다.</br>
