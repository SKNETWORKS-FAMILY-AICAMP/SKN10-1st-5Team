...
중앙: 연도별 자동차 등록 대수 조회, 기업별FAQ 조회을 눌러도 각각 페이지로 이동함
...
카테고리가 바뀌고 그 기업별 카테고리가 선택할 수 있음</br>
...
중앙: 그래프를 선택하면 연도 범위 선택, 차량 종류를 선택할 수 있는 화면이 나옴</br>
연도와 차량 종류를 선택하면 가지고 있는 데이터를 기반으로 그래프를 만들어줌</br>
